...
int* moveWithProbability(int x, int y, struct Point idealV, float bresenhamP, float * prob) {
	...
	*/
	...
	} else if (*prob <= bresenhamP + (0.0 * p)) { // probability of moving in direction 3 unit away from ideal
		switch(ideal) {
			case 0: // Ideal N
				x += 1;
				y -= 1;
	...
	} else if (*prob <= bresenhamP + (0.0 * p)) { // probability of moving in direction 4 unit away from ideal
		switch(ideal) {
			case 0: // Ideal N
				y -= 1;
				ratios[4]++;
	...
	} else if (*prob <= bresenhamP + (0.0 * p)) { // probability of moving in direction 5 unit away from ideal
		switch(ideal) {
			case 0: // Ideal N
				x -= 1; 
				y -= 1; 
